local setparent
pcall(function()
	setparent = sethiddenproperty or set_hidden_property
end)
if not setparent then
...
local function ProtectGui(g)
	local suc
	if typeof(gethui or false) == "function" then
		suc = pcall(function() setparent(g, gethui()) end)
	elseif type(syn) == "table" and syn and syn.protect_gui then
		suc = pcall(function() syn.protect_gui(g) end)
	else
		suc = pcall(function() setparent(g, game:GetService("CoreGui")) end)
	end
	if not suc then
		pcall(function()
...
local function DrawDistance(plr, color)
	...
	local head = plr.Character.Head.Position
	local lp = game.Players.LocalPlayer
	local plrpos = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
...
local function CreatePasswordMenu(onSuccess)
	for _,g in ipairs(game:GetService("CoreGui"):GetChildren()) do
		if g.Name == "NebulaPasswordMenu" then g:Destroy() end
	end
